...
- 1. - |                           知识领域                           | 题目                                                         | 考察能力项 | 难度 | 标准答案                                                     | 提供者 |
       | :----------------------------------------------------------: | :----------------------------------------------------------- | :--------- | :--- | :----------------------------------------------------------- | :----- |
       |                         编程语言基础                         | 介绍Java 内存分代结构追问:  分配过程是怎样的？追问:  常用的回收算法有哪些？追问:  CMS 、G1回收器的区别 | Java 基础  | 中等 |                                                              |        |
       | Java hashmap的实现原理追问:  红黑树的特性有哪些，时间复杂度如何追问:  hashmap线程不安全在哪？追问: ConcurrentHashMap 是如何做到线程安全的? | Java 基础                                                    | 简单       |      |                                                              |        |
       |                                                              |                                                              |            |      |                                                              |        |
       |                            数据库                            |                                                              |            |      |                                                              |        |
       |                            中间件                            |                                                              |            |      |                                                              |        |
       |                           操作系统                           | 什么是BIO、NIO、AIO追问:  select、poll、epoll区别追问:  是否了解reactor 和 Proactor 模型 | IO相关     | 中等 |                                                              |        |
       | Linux查看性能指标的命令有哪些？追问：有哪些关键指标？追问：进程内核态和用户态有什么区别，为什么做这样划分？追问：cpu load和busy指标高分别代表什么含义？追问：*零拷贝*(Zero-copy)有哪些实现方式？追问：DMA是什么？ | Linux                                                        |            |      |                                                              |        |
       | 进程和线程的区别？追问:  多进程 vs 多线程对比追问:  什么是僵尸进程、孤儿进程，有什么危害追问： 有哪些IPC方式，各种对比？ |                                                              |            |      |                                                              |        |
       |                             网络                             | 解释下TCP三次握手，四次挥手过程？追问:  为什么要维护TIME_WAIT 状态追问:  Nagle 算法了解吗，它作用是啥 | TCP        | 低   | 三次握手：客户端发送SYN包给服务端，客户端状态变为：SYN_SEND;【SYN】SYN=1，seq=x；服务端接受到SYN报文后进行确认，状态变为：SYN_RECV;【SYN+ACK】SYN=1，seq=y，ACK=1(acknowledgement，0-无效，1-有效)，ack=x+1(acknowledge number，为上一次请求的seq+1);客户端收到【SYN+ACK】包后进行确认，状态变为ESTABLISHED;【ACK】ACK=1，ack=y+1；四次挥手：客户端发送一个FIN报文给服务端，客户端状态变为FIN_WAIT_1;(FIN，seq=x+2，ack=y+1)服务端接受到报文后向客户端回一个ACK报文段，客户端进入FIN_WAIT_2状态，服务端进入CLOSE_WAIT状态；(ack=x+3)服务端向客户端发送FIN报文段，请求关闭连接，状态变为LAST_ACK; (FIN，seq=y+1)客户端收到服务端FIN报文，向服务端发送ACK报文，然后状态变为TIME_WAIT，服务端收到报文后关闭连接，客户端等待2msl(报文最大生存时)后没有收到回复，则关闭连接；(ack=y+2) |        |
       |             在浏览器输入 URL 回车之后发生了什么              |                                                              |            |      | URL 解析 DNS 查询 TCP 连接 处理请求 接受响应 渲染页面        |        |
       |                           方案设计                           |                                                              |            |      |                                                              |        |
       |                           编程算法                           | 两个链表生成相加链表                                         | 链表       | 基础 |                                                              |        |
       |                           两数之和                           | 哈希                                                         | 基础       |      |                                                              |        |
